...
For two tasks if they share one or more same elements within their evidential bases, their evidence is **overlapped** i.e. they do not have disjoint evidential base. Ideally when combining evidence, and if some portion of evidence is present more than once, it should be subtracted from final result, however since there is impossible to determine exact amount of some portion of evidence influencing certain term this **cannot be done**. **Therefore revision rule applies only if tasks have disjoint evidential bases.**
Given that the purpose of a revision rule is to combine evidence, it is important not to sum some evidence more than once when making revision. However once computaton  
